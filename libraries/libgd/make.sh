...
package_configure $EXTRACONF --disable-shared --enable-static --with-png=$SYS_PREFIX --with-freetype=$SYS_PREFIX --with-jpeg=$SYS_PREFIX --without-avif --without-heif --with-tiff=$SYS_PREFIX --with-webp=$SYS_PREFIX --without-xpm --without-fontconfig --without-x
